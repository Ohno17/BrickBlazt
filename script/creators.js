...
function createLeadingLetterRequirement(letter) {
    return createCustomRequirement((input) => input[0] == letter, `Word must start with "${letter}"`);
}

function createTrailingLetterRequirement(letter) {
    return createCustomRequirement((input) => input.endsWith(letter), `Word must end with "${letter}"`);
}

function createContainingLetterRequirement(letter) {
    return createCustomRequirement((input) => input.includes(letter), `Word must contain "${letter}"`);
}
